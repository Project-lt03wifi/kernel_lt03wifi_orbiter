...
#define LULZACTIVE_TUNER "DerTeufel1980 - CastagnaIT - Christopher83"
...
static inline void fix_screen_off_min_step(struct cpufreq_lulzactive_cpuinfo *pcpu) {
	...
	if (SCREEN_OFF_LOWEST_STEP == screen_off_min_step)
...
static void cpufreq_lulzactive_timer(unsigned long data)
{
	...
	u64 time_in_idle;
	u64 idle_exit_time;
	struct cpufreq_lulzactive_cpuinfo *pcpu = &per_cpu(cpuinfo, data);
	u64 now_idle;
	...
	while (1) {
		...
		for_each_cpu(cpu, &tmp_mask) {
			...
			__cpufreq_driver_target(pcpu->policy,
					pcpu->target_freq,
					CPUFREQ_RELATION_H);
			pcpu->freq_change_time_in_idle = get_cpu_idle_time_us(cpu, &pcpu->freq_change_time);
